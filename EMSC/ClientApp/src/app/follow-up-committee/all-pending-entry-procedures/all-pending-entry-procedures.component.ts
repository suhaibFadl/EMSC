...
@Component({
  selector: 'app-all-pending-entry-procedures',
  templateUrl: './all-pending-entry-procedures.component.html',
  styleUrls: ['./all-pending-entry-procedures.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class AllPendingEntryProceduresComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.GetAllPendingEntryProcedures();
  }
  ...
  GetAllPendingEntryProcedures() {
    this.fcs.clearCache();
    this.fcs.GetAllPendingEntryProcedures().subscribe(data => {
      ...
      this.dataSource.filterPredicate = ((data, filter) => {
        ...
        const g = !filter.flightDate || moment(data.flightDate).format('yyyy-MM-DD').toLowerCase().includes(moment(filter.flightDate).format('yyyy-MM-DD'));
        const f = !filter.userDate || moment(data.userDate).format('yyyy-MM-DD').toLowerCase().includes(moment(filter.userDate).format('yyyy-MM-DD'));
