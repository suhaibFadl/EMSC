...
@Component({
  selector: 'app-trveling-procedures-back',
  templateUrl: './trveling-procedures-back.component.html',
  styleUrls: ['./trveling-procedures-back.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class TrvelingProceduresBackComponent implements OnInit {
  ...
  displayedColumns2: string[] = ['index', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'ehigth', 'ninth','ten'];
  ...
  ngOnInit(): void {
    ...
    this.GetTravelingProceduresByUserId();
    this.GetAllPatsRejected();
  ...
  GetAllPatsRejected() {
    this.treat.clearCache();
    this.treat.GetAllPatsRejected().subscribe(data => {
  ...
  //Travel update modal
  onUpdateModal(edittravel: TravelBack): void {
    ...
    this.tr.UpdateTravelBack(edittravel.id, edittravel).subscribe(
      result => {
        ...
        switch (this.UserRole) {
          case "مسؤول التسفير": this.GetTravelingProceduresByUserId();
            break;
        }
  ...
  //Travel delete modal
  onDeleteModal(traveldelete: TravelBack) {
