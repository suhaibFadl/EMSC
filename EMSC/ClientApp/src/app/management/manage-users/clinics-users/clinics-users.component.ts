...
@Component({
  selector: 'app-clinics-users',
  templateUrl: './clinics-users.component.html',
  styleUrls: ['./clinics-users.component.css']
})
export class ClinicsUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.roles$ = this.rol.GetRolesForHospitalsInside();
  ...
  loadAllUsers() {
    this.acct.GetUserGetInsideHospital().subscribe(data => {
  ...
  onSubmit() {
    ...
    this.acct.AddAccountHo(newpAccount).subscribe(
      result => {
        this.acct.clearCache();
        this.loadAllUsers();
