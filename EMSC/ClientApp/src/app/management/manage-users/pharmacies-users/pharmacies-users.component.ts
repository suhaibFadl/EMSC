...
@Component({
  selector: 'app-pharmacies-users',
  templateUrl: './pharmacies-users.component.html',
  styleUrls: ['./pharmacies-users.component.css']
})
export class PharmaciesUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.PharmacyId = new FormControl('', [Validators.required]);
    ...
    this.roles$ = this.rol.GetRolesForPharmacies();
  ...
  loadAllUsers() {
    this.acct.GetUsersPharmacies().subscribe(data => {
  ...
  onSubmit() {
    ...
    this.acct.AddAccountPharmacy(newpAccount).subscribe(
      result => {
        this.acct.clearCache();
        this.loadAllUsers();
