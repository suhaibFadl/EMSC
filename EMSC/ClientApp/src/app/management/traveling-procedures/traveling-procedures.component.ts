...
@Component({
  selector: 'app-traveling-procedures',
  templateUrl: './traveling-procedures.component.html',
  styleUrls: ['./traveling-procedures.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class TravelingProceduresComponent implements OnInit {
  ...
  dataSource2!: MatTableDataSource<TravelingPr>;

  displayedColumns: string[] = ['index', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'ehigth', 'ninth', 'ten', 'eleventh', 'twelve','thirteen', 'edit', 'delete'];
  displayedColumns1: string[] = ['index', 'first', 'second', 'third', 'third1', 'third2', 'fourth', 'fifth', 'sixth', 'seventh', 'eight'];
  displayedColumns2: string[] = ['index', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eihgth', 'ninth', 'ten', 'eleventh','twelve','thirteen'];
  displayedColumns3: string[] = ['index', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eihgth', 'ninth', 'ten', 'eleventh','twelve'];
  displayedColumns4: string[] = ['index', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'ehigth', 'ninth', 'ten', 'eleventh', 'edit', 'delete'];
 ...
 // BranchUserId!: string;
  ...
  ngOnInit(): void {
    ...
    switch (this.UserRole) {
      case "مسؤول التسفير": this.GetTraveling();
        break;
      case "مدير الفرع": this.GetTravelingProceduresByBranchId();
        break;
      case "موظف إدخال الفرع": this.GetTravelingProceduresByBranchId();
        break;
      case "الإدارة": this.GetAllTravelingProceduresManagement();
        break;
      case "مشرف إداري": this.GetTravelingProceduresByCountryId();
        break;      
    }
  ...
  //  عرض كافة إجراءات التسفير
  GetTraveling() {
    this.tr.clearCache();
    this.tr.GetTraveling().subscribe(data => {
  ...
  //عرض إجراءات التسفير التابعين للفرع
  GetTravelingProceduresByBranchId() {
    this.tr.clearCache();
    this.tr.GetTravelingProceduresByBranchId(this.BranchUserId.toString()).subscribe(data => {
      this.dataSource = new MatTableDataSource(data);
      this.dataSource.paginator = this.paginator;
  ...
  //عرض إجراءات التسفير حسب دولة العلاج
  GetTravelingProceduresByCountryId() {
    this.tr.clearCache();
    this.tr.GetTravelingProceduresByCountryId(this.UserCountryId.toString()).subscribe(data => {
  ...
  //عرض كافة إجراءات التسفير للفرع الرئيسي 
  GetAllTravelingProceduresManagement() {
    this.tr.clearCache();
    this.tr.GetAllTravelingProceduresManagement().subscribe(data => {
      this.dataSource = new MatTableDataSource(data);
      this.dataSource.paginator = this.paginator;
  ...
  onUpdate() {
    ...
    this.tr.updateTraveling(edittravel.id, edittravel).subscribe(
      result => {
        this.tr.clearCache();
        switch (this.UserRole) {
          case "مسؤول التسفير": this.GetTraveling();
  ...
  onDelete(): void {
    ...
    this.tr.DeleteTraveling(traveldelete.id, this.DidTr.value).subscribe(result => {
      this.tr.clearCache();
      switch (this.UserRole) {
        case "مسؤول التسفير": this.GetTraveling();
          break;
