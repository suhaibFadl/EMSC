...
export class PatientsListAcceptedComponent implements OnInit {
  ...
  values = [
    { value: 3, title: 'قبول' },
    { value: 4, title: 'رفض' },
    { value: 6, title: 'انتظار' }
  ];
  favoriteSeason: number = 3;;
  ...
  GetAllPatientsIsAcceptedByManagment() {
    this.pa.clearCache();
    this.pa.GetAllPatientsIsAcceptedByManagment(this.CUserId.toString()).subscribe(data => {
  ...
  GetPatAttendantByTRID(id) {
    this.pa.clearCache();
    this.pa.GetPatAttendantByTRIDAccepted(id).subscribe(data => {
  ...
  onReplyModal(rpid: PTransOutside): void {
    ...
    this.pa.GetPatAttendantByTRIDAccepted(this.TRId.value).subscribe(data => {
  ...
  ViewlAttendantData(p: PTransOutside) {
    this.GetPatAttendantByTRID(p.id)
